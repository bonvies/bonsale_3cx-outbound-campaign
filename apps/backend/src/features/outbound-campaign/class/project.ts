...
export default class Project {
  ...
  static async initOutboundProject(projectData: {
    ...
    try {
      ...
      if (existingProject) {
        console.log(`專案 ${projectId} 已存在，更新 token 並返回實例`);
        ...
        console.log(`專案 ${projectId} token 已更新`);
        return existingProject;
      }

      // 創建新專案
      console.log(`開始初始化新專案 ${projectId}`);
      ...
      console.log(`專案 ${projectId} 初始化完成並儲存到 Redis`);
      return project;

    } catch (error) {
      console.error(`初始化專案 ${projectId} 失敗:`, error);
      throw error;
    }
  }
  ...
  async updateState(newState: 'active' | 'stop'): Promise<void> {
    ...
    try {
      ...
      console.error(`更新專案狀態到 Redis 失敗:`, error);
    }
  }
  ...
  async setError(errorMessage: string): Promise<void> {
    ...
    console.error(`專案 ${this.projectId} 發生錯誤: ${errorMessage}`);
    ...
    try {
      ...
      if (this.broadcastWsRef) {
        try {
          ...
          console.log(`錯誤已廣播給客戶端 - 專案: ${this.projectId}`);
        } catch (broadcastError) {
          console.error(`廣播錯誤訊息失敗:`, broadcastError);
        }
      }
    } catch (error: unknown) {
      console.error(`更新專案錯誤到 Redis 失敗:`, error);
    }
  }
  ...
  async clearError(): Promise<void> {
    if (this.error) {
      console.log(`專案 ${this.projectId} 錯誤已解決，清除錯誤狀態`);
      this.error = null;
      ...
      try {
        ...
        if (this.broadcastWsRef) {
          try {
            await broadcastAllProjects(this.broadcastWsRef, this.projectId);
            console.log(`錯誤清除已廣播給客戶端 - 專案: ${this.projectId}`);
          } catch (broadcastError) {
            console.error(`廣播錯誤清除訊息失敗:`, broadcastError);
          }
        }
      } catch (error: unknown) {
        console.error(`清除專案錯誤到 Redis 失敗:`, error);
      }
    }
  }
  ...
  async setInfo(infoMessage: string): Promise<void> {
    ...
    console.log(`專案 ${this.projectId} 資訊: ${infoMessage}`);
    ...
    try {
      ...
      if (this.broadcastWsRef) {
        try {
          await broadcastAllProjects(this.broadcastWsRef, this.projectId);
          console.log(`資訊已廣播給客戶端 - 專案: ${this.projectId}`);
        } catch (broadcastError) {
          console.error(`廣播資訊訊息失敗:`, broadcastError);
        }
      }
    } catch (error: unknown) {
      console.error(`更新專案資訊到 Redis 失敗:`, error);
    }
  }
  ...
  async clearInfo(): Promise<void> {
    if (this.info) {
      console.log(`專案 ${this.projectId} 資訊已清除`);
      this.info = null;
      ...
      try {
        ...
        if (this.broadcastWsRef) {
          try {
            await broadcastAllProjects(this.broadcastWsRef, this.projectId);
            console.log(`資訊清除已廣播給客戶端 - 專案: ${this.projectId}`);
          } catch (broadcastError) {
            console.error(`廣播資訊清除訊息失敗:`, broadcastError);
          }
        }
      } catch (error: unknown) {
        console.error(`清除專案資訊到 Redis 失敗:`, error);
      }
    }
  }
  ...
  async setWarning(warningMessage: string): Promise<void> {
    ...
    console.log(`專案 ${this.projectId} 警告: ${warningMessage}`);
    ...
    try {
      ...
      if (this.broadcastWsRef) {
        try {
          await broadcastAllProjects(this.broadcastWsRef, this.projectId);
          console.log(`警告已廣播給客戶端 - 專案: ${this.projectId}`);
        } catch (broadcastError) {
          console.error(`廣播警告訊息失敗:`, broadcastError);
        }
      }
    } catch (error: unknown) {
      console.error(`更新專案警告到 Redis 失敗:`, error);
    }
  }
  ...
  async clearWarning(): Promise<void> {
    if (this.warning) {
      console.log(`專案 ${this.projectId} 警告已清除`);
      this.warning = null;
      ...
      try {
        ...
        if (this.broadcastWsRef) {
          try {
            await broadcastAllProjects(this.broadcastWsRef, this.projectId);
            console.log(`警告清除已廣播給客戶端 - 專案: ${this.projectId}`);
          } catch (broadcastError) {
            console.error(`廣播警告清除訊息失敗:`, broadcastError);
          }
        }
      } catch (error: unknown) {
        console.error(`清除專案警告到 Redis 失敗:`, error);
      }
    }
  }
  ...
  private async processWebSocketMessage(broadcastWs: WebSocketServer, data: Buffer): Promise<void> {
    try {
      ...
      console.log(`WebSocket 訊息處理 (throttled) - 事件類型: ${messageObject.event?.event_type}`);
      ...
      switch (eventType) {
        case 0:
          console.log(`狀態 ${eventType}:`, messageObject.event);
          ...
          try {
            if (eventEntity && this.access_token) {
              ...
              if (participantResult.success) {
                ...
                console.log(`✅ 捕獲 participant 快照 - entity: ${eventEntity}`);
              } else {
                console.log(`⚠️ 無法獲取 participant 快照 - 對方可能已掛斷: ${eventEntity}`);
                participantSnapshot0 = participantResult; // 關鍵：即使失敗也保存 因為對方已掛斷
              }
            }
          } catch (captureError) {
            console.log(`⚠️ 捕獲 participant 快照失敗:`, captureError);
          }
          ...
          if (this.state === 'stop') {
            console.log(`專案狀態為 stop，執行停止狀態邏輯處理`);
          ...
          if (this.throttledOutboundCall) {
            ...
            if (result0) {
              result0.catch(error => {
                console.error('case 0 觸發外撥邏輯時發生錯誤:', error);
              });
            }
          }
          break;
        case 1:
          ...
          console.log(`調用 outboundCall 處理事件 entity: ${eventEntity}，狀態: ${this.state}`);
          // 注意：不 await，讓它在背景執行，避免在 WebSocket 事件處理器內造成死鎖
          this.outboundCall(broadcastWs, eventEntity, true, false, participantSnapshot1).catch(error => {
            console.error('case 1 觸發外撥邏輯時發生錯誤:', error);
          });
      ...
      console.log('3CX WebSocket 收到非JSON訊息:', data.toString('utf8'));
  ...
  private async recordCallerExtensionLastExecutionTime(dn: string): Promise<void> {
    if (dn) {
      ...
      console.log(`📝 紀錄分機 ${dn} 最後執行時間: ${this.callerExtensionLastExecutionTime[dn]}`);

      // 專案已移除時禁止 saveProject：它會整包 hSet 重建 key，並把 projectId sAdd 回 active_projects
      if (this.isRemoved) {
        console.log(`專案 ${this.projectId} 已完全停止並移除，跳過保存分機執行時間`);
        return;
      }
      ...
      try {
        ...
        console.log(`✅ 分機 ${dn} 執行時間已保存到 Redis`);
      } catch (error) {
        console.error(`❌ 保存分機 ${dn} 執行時間到 Redis 失敗:`, error);
      }
    }
  }
  ...
  private async outboundCall(
    ...
    console.log(`[🔴 outboundCall 嘗試獲取 Mutex] eventEntity: ${eventEntity}`);

    await this.processCallerMutex.runExclusive(async () => {
      const mutexAcquiredTime = Date.now();
      console.log(`[🔴 outboundCall 已獲取 Mutex] 等待時間: ${mutexAcquiredTime - outboundCallStartTime}ms`);

      try {
        console.log('[🔴 outboundCall 開始執行邏輯]', {
          eventEntity,
          isExecuteOutboundCalls,
          isInitCall
        });

        // 步驟一: 檢查專案狀態 專案已從 Redis 移除，任何後續處理都會把已刪除的 key 寫回，直接跳過
        if (this.state !== 'active' || this.isRemoved) {
          console.log('專案狀態不符合外撥條件:', this.state, { isRemoved: this.isRemoved });
          return;
        }
        ...
        if (currentToken && currentToken !== this.access_token) {
          ...
          console.log('⚠️ Token 已更新，將在 Mutex 釋放後重新建立 WebSocket 連接');

          // 🆕 使用 Flag 防止重複刷新 WebSocket 連接
          if (!this.isRefreshingToken) {
            this.isRefreshingToken = true;  // 🔒 立即鎖定
            console.log('🔒 設置 isRefreshingToken = true，防止重複刷新');

            // 使用 setImmediate 延遲執行，確保 Mutex 先釋放
            setImmediate(() => {
              this.handleTokenUpdateWebSocketReconnect(broadcastWs)
                .catch(error => {
                  console.error('Token 更新後非同步重連 WebSocket 失敗:', error);
                })
                .finally(() => {
                  this.isRefreshingToken = false;  // 🔓 解鎖
                  console.log('🔓 設置 isRefreshingToken = false，允許下次刷新');
                });
            });
          } else {
            console.log('⏭️ 已有 WebSocket 重連接在進行中，跳過此次刷新');
          }
          // 注意：分機狀態管理器現在使用管理員 token 自動管理，不需要同步更新
        }
        ...
        if (broadcastWs) {
          try {
            ...
            console.error('廣播錯誤資訊失敗:', broadcastError);
          }
        }
      ...
      console.log(`[🔴 outboundCall Mutex 釋放] 總耗時: ${mutexReleaseTime - outboundCallStartTime}ms`);
    });
  }
  ...
  private async updateCallerInfo(): Promise<void> {
    try {
      ...
      console.log(`專案 ${this.projectId} 的 caller 資訊已更新到 Redis`);

    } catch (error) {
      console.error('更新 caller 資訊失敗:', error);
      throw error;
    }
  }
  ...
  private async updateLatestCallRecordStatus(): Promise<void> {
    try {
      ...
      for (let i = 0; i < this.latestCallRecord.length; i++) {
        ...
        if (callerInfo && callerInfo.participants && callerInfo.participants.length > 0) {
          ...
          if (currentCall.status !== newStatus) {
            ...
            console.log(`撥打狀態更新 - 分機: ${currentCall.dn}, 客戶: ${currentCall.memberName}, 狀態: ${oldStatus} -> ${newStatus}`);
          }
        }
      }
      ...
      console.error('更新當前撥打記錄狀態失敗:', error);
      // 不拋出錯誤，避免影響主要流程
    }
  }
  ...
  private async executeOutboundCalls(
    ...
    console.log(`[🟢 executeOutboundCalls 開始] eventEntity: ${eventEntity}, isInitCall: ${isInitCall}`);

    // 檢查是否有分機
    if (!this.caller || this.caller.length === 0) {
      console.error('當前專案沒有分機');
      return;
    }
    ...
    if (this.recurrence) {
      ...
      if (!isInSchedule) {
        console.warn(`今天不在 recurrence 排程內，跳過外撥`);
    ...
    if (this.callRestriction && this.callRestriction.length > 0) {
      ...
      if (isInRestrictedTime) {
        console.warn(`當前時間在限制撥打時間內，跳過外撥`);
    ...
    if (!this.access_token) {
      console.log(`無效的 access_token，跳過分外撥`);
    ...
    // 檢查是否為初始撥號
    if (isInitCall) {
      console.log(`初始撥號，開始對所有分機進行外撥檢查`);
      ...
      for (const caller of callerSnapshot) {
        try {
          // 檢查代理人用戶是否忙碌
          if (!this.access_token) {
            console.log(`無效的 access_token，跳過分機 ${caller.dn} 的外撥`);
            continue;
          }
          ...
          if (participants && participants.length > 0) {
            console.log(`分機 ${caller.dn} 有 ${participants.length} 個通話中，跳過外撥`);
            continue;
          }

          const agentUser = await getUsers(this.access_token, caller.dn);
          if (!agentUser.success) {
            console.log(`無法獲取分機 ${caller.dn} 的代理人用戶資訊，跳過外撥`);
            continue;
          }
          ...
          if (CurrentProfileName) {
            ...
            if (isAgentUserBusy) {
              console.log(`分機 ${caller.dn} 的代理人用戶忙碌，跳過外撥`);
              continue;
            }
          }
          ...
          if (currentIndex < callerSnapshot.length - 1) {
            console.log(`⏳ 處理完分機 ${caller.dn}，等待 1000ms 後處理下一個分機`);
            await this.delay(1000); // 1000ms 延遲
          }

        } catch (callerError) {
          console.error(`處理分機 ${caller.dn} 初始外撥時發生錯誤:`, callerError);
          // 繼續處理下一個分機
        }
      }
    } else {
      ...
      if (!participant.success) {
        ...
        if (!callControls.success) {
          console.error(`無法獲取事件實體 ${eventEntity} 的呼叫控制資訊，跳過外撥`);
        ...
        if (!callControl) {
          console.error(`無法在呼叫控制清單中找到分機 ${eventEntity_dn}，跳過外撥`);
          this.setError(`無法在呼叫控制清單中找到分機 ${eventEntity_dn}，跳過外撥`);
          return;
        }
        const participants = callControl.participants;
        if (participants && participants.length > 0) {
          console.warn(`分機 ${eventEntity_dn} 仍有參與者，代理人用戶可能仍忙碌，跳過外撥`);
        ...
        if (!agentUser.success) {
          console.error(`無法獲取分機 ${eventEntity_dn} 的代理人用戶資訊，跳過外撥`);
        ...
        if (CurrentProfileName) {
          const isAgentUserBusy = CurrentProfileName !== "Available";
          if (isAgentUserBusy) {
            console.warn(`分機 ${eventEntity_dn} 的代理人用戶忙碌，跳過外撥`);
        ...
        // 代理人可用，執行外撥邏輯
        console.log(`分機 ${eventEntity_dn} 的代理人用戶可用，繼續外撥流程`);
        const currentDeviceId = callControl.devices[0]?.device_id;
        await this.processCallerOutbound(eventEntity_dn, currentDeviceId);
      } else {
        console.log(`成功獲取事件實體 ${eventEntity} 的參與者資訊:`, participant.data);
        if (!participant.data) {
          console.error(`參與者資料不完整, 無法進行外撥`);
    ...
    console.log(`[🟢 executeOutboundCalls 完成] 耗時: ${executeEndTime - executeStartTime}ms`);
  }
  ...
  private async processCallerOutbound(dn: string, deviceId: string): Promise<void> {
    ...
    console.log(`[🔵 processCallerOutbound 開始] dn: ${dn}, deviceId: ${deviceId}`);

    if (!dn || !deviceId) {
      console.error('分機或設備 ID 未定義，無法進行外撥處理');
      return;
    }
    ...
    try {
      ...
      console.log(`[🔵 processCallerOutbound] 準備從 Redis 取得下一通電話...`);
      ...
      console.log(`[🔵 processCallerOutbound] 從 Redis 取得下一通電話完成，耗時: ${getNextCallEndTime - getNextCallStartTime}ms`);
      ...
      if (nextCallItem) {
        ...
        if (existingCallIndex >= 0) {
          ...
          if (existingCall) {
            ...
            if (prevCallIndex >= 0) {
              ...
              if (oldRecord) {
                console.log(`⚠️ 偵測到分機 ${dn} 有未處理的舊記錄 - 客戶: ${oldRecord.memberName} (${oldRecord.customerId}), 立即處理以避免遺失`);

                try {
                  // 立即處理舊記錄
                  await this.recordBonsaleCallResult(oldRecord);
                  console.log(`✅ 已處理分機 ${dn} 的舊記錄 - 客戶: ${oldRecord.memberName} (${oldRecord.customerId})`);
                } catch (error) {
                  console.error(`❌ 處理分機 ${dn} 的舊記錄時發生錯誤:`, error);
                  // 即使處理失敗，也繼續執行，避免阻塞流程
                }
              }
            ...
            console.log(`保存分機 ${dn} 的前一筆撥打記錄 - 客戶: ${existingCall.memberName} (${existingCall.customerId})`);
          }
        }
        ...
        // 有撥號名單，進行撥打
        console.log(`準備撥打 - 客戶: ${nextCallItem.memberName} (${nextCallItem.customerId}), 電話: ${nextCallItem.phone}, 分機: ${dn}`);
        await this.makeOutboundCall(dn, deviceId, nextCallItem.phone, 2000);
      } else {
        // 沒有撥號名單，但要檢查該分機是否有當前撥打記錄需要處理
        console.log(`專案 ${this.projectId} 的撥號名單已空，分機 ${dn} 暫無可撥打號碼`);
        ...
        if (existingCallIndex >= 0) {
          ...
          if (existingCall) {
            ...
            console.log(`保存分機 ${dn} 的最後一筆撥打記錄到 previousCallRecord - 客戶: ${existingCall.memberName} (${existingCall.customerId})`);
          }
        }
    ...
    console.log(`[🔵 processCallerOutbound 完成] dn: ${dn}, 耗時: ${processEndTime - processStartTime}ms`);
  }
  ...
  private async makeOutboundCall(dn: string, deviceId: string, targetNumber: string | null, delayMs: number = 1000): Promise<void> {
    ...
    console.log(`[🟡 makeOutboundCall 開始] dn: ${dn}, targetNumber: ${targetNumber}`);
    ...
    try {
      ...
      // 添加延遲
      console.log(`[🟡 makeOutboundCall] 等待 ${delayMs}ms 後撥打電話: ${dn} -> ${targetNumber}`);
      ...
      if (this.previousCallRecord && this.previousCallRecord.length > 0) {
        ...
        if (previousCallIndex >= 0) {
          ...
          if (previousCallForThisExtension) {
            ...
            console.log(`[🟡 makeOutboundCall] 準備記錄前一通電話結果 - 客戶: ${previousCallForThisExtension.memberName} (${previousCallForThisExtension.customerId})`);
            const recordStartTime = Date.now();
            await this.recordBonsaleCallResult(previousCallForThisExtension);
            const recordEndTime = Date.now();
            console.log(`[🟡 makeOutboundCall] 記錄前一通電話結果完成，耗時: ${recordEndTime - recordStartTime}ms`);

            // 處理完成後，從 previousCallRecord 中移除該記錄，避免重複處理
            this.previousCallRecord.splice(previousCallIndex, 1);
            console.log(`已移除分機 ${dn} 的已處理記錄，剩餘 previousCallRecord: ${this.previousCallRecord.length} 筆`);
          }
        }
      }
      if (!targetNumber) {
        console.log(`[🟡 makeOutboundCall] 分機 ${dn} 無撥號名單，跳過撥打`);
        return;
      }

      // 發起外撥
      console.log(`[🟡 makeOutboundCall] 準備發起外撥: ${dn} -> ${targetNumber}`);
      const callStartTime = Date.now();
      await makeCall(this.access_token, dn, deviceId, "outbound", targetNumber);
      const callEndTime = Date.now();
      console.log(`[🟡 makeOutboundCall] 成功發起外撥，耗時: ${callEndTime - callStartTime}ms - ${dn} -> ${targetNumber}`);

      const makeCallEndTime = Date.now();
      console.log(`[🟡 makeOutboundCall 完成] 總耗時: ${makeCallEndTime - makeCallStartTime}ms`);
    } catch (error) {
      const errorMsg = `外撥失敗 ${dn} -> ${targetNumber}: ${error instanceof Error ? error.message : String(error)}`;
      await this.setError(errorMsg);
      console.error(`[🟡 makeOutboundCall] 外撥失敗 ${dn} -> ${targetNumber}:`, error);
      const makeCallEndTime = Date.now();
      console.log(`[🟡 makeOutboundCall 失敗] 總耗時: ${makeCallEndTime - makeCallStartTime}ms`);
      throw error;
    }
  }
  ...
  private async recordBonsaleCallResult(previousCallRecord: CallRecord): Promise<void> {
    const recordStartTime = Date.now();
    console.log(`[🟢 recordBonsaleCallResult 開始] 客戶: ${previousCallRecord?.memberName} (${previousCallRecord?.customerId}), 分機: ${previousCallRecord?.dn}`);

    try {
      // 這裡可以根據當前的 caller 狀態來判斷前一通電話的通話結果
      if (!previousCallRecord) {
        console.warn('沒有前一筆撥打記錄可供寫入 Bonsale');
        return;
      }
      console.log(`[🟢 recordBonsaleCallResult] 準備記錄 Bonsale 通話結果 - 客戶: ${previousCallRecord.memberName} (${previousCallRecord.customerId}), 分機: ${previousCallRecord.dn}`);
      ...
      switch (status) {
        case "Dialing":
          console.log(`分機 ${previousCallRecord.dn} 狀態為撥號中，前一通電話記錄為未接通`);
          try {
            // 紀錄分機最後執行時間
            ...
            if ((!previousCallRecord.description || previousCallRecord.description.trim() === '')
              ...
              console.warn(`分機 ${previousCallRecord.dn} 的前一筆撥打記錄沒有 description 或 description2 描述資訊`);
            } else {
              // 有描述資訊才呼叫 post9000 API 和 post9000Dummy API
              // 因為這是 21 世紀 需要 post9000 回去的紀錄 
              // post9000Dummy 是 PY 需要的紀錄

              console.log(`分機 ${previousCallRecord.dn} 的前一筆撥打記錄有 description 和 description2 描述資訊`);

              // post9000 重試邏輯：最多嘗試 3 次
              let post9000Success = false;
              for (let tryPost9000Times = 1; tryPost9000Times <= 3; tryPost9000Times++) {
                try {
                  console.log(`嘗試呼叫 post9000 (第 ${tryPost9000Times} 次) - 電話: ${previousCallRecord.phone}`);
                  ...
                  if (post9000Result.success) {
                    ...
                    if (isApiSuccess) {
                      console.log(`✅ post9000 成功 (第 ${tryPost9000Times} 次嘗試) - 電話: ${previousCallRecord.phone} - ${JSON.stringify(post9000Result.data)}`);
                      ...
                      if (tryPost9000Times < 3) {
                        console.log(`⏳ 等待 2 秒後重試 post9000 - 電話: ${previousCallRecord.phone}`);
                        await this.delay(2000); // 等待 2 秒後重試
                      } else {
                        console.error(`❌ post9000 已達最大重試次數 (${tryPost9000Times} 次)，停止嘗試 - 電話: ${previousCallRecord.phone} - StatusCode=${apiData?.StatusCode}, Message=${apiData?.Message}`);
                      }
                    }
                  } else {
                    ...
                    if (tryPost9000Times < 3) {
                      ...
                      console.error(`❌ post9000 已達最大重試次數 (${tryPost9000Times} 次)，停止嘗試 - 電話: ${previousCallRecord.phone} - ${post9000Result.error?.error || '未知錯誤'}`);
                    }
                  }
                } catch (error) {
                  ...
                  if (tryPost9000Times < 3) {
                    ...
                    console.error(`❌ post9000 異常已達最大重試次數 (${tryPost9000Times} 次)，停止嘗試 - 電話: ${previousCallRecord.phone}`);
                  }
                }
              }
              ...
              if (post9000Success) {
                try {
                  console.log(`🔄 post9000 成功，開始呼叫 post9000Dummy`);
                  ...
                  if (dummyResult.success) {
                    console.log(`✅ post9000Dummy 成功, ${JSON.stringify(dummyResult.data)}`);
                  } else {
                ...
                console.warn(`⚠️ post9000 失敗，跳過 post9000Dummy 的呼叫`);
              }
            }
          } catch (error) {
            ...
            try {
              ...
              console.error(`❌ 移除撥號名單項目時發生錯誤: ${removeError instanceof Error ? removeError.message : String(removeError)}`);
            }
          }
          break;
        case "Connected":
          console.log(`分機 ${previousCallRecord.dn} 狀態為已接通，前一通電話記錄為已接通`);
          ...
          try {
            ...
            setTimeout(async () => {
              try {
                ...
                await this.setError(errorMsg);
                console.log('❌ updateVisitRecord 異常:', {
                  projectId: this.projectId,
                  callFlowId: this.callFlowId,
                  state: this.state,
                  client_id: this.client_id,
          ...
          break;
        default:
          console.warn(`分機 ${previousCallRecord.dn} 狀態為未知，無法記錄前一通電話結果`);
          // 即使狀態未知，也要移除使用過的撥號名單項目，避免名單殘存
          try {
            await CallListManager.removeUsedCallListItem(previousCallRecord.projectId, previousCallRecord.customerId);
            console.log(`🗑️ 已移除未知狀態的撥號名單項目 - 專案: ${previousCallRecord.projectId}, 客戶: ${previousCallRecord.customerId}`);
          } catch (removeError) {
            console.error(`❌ 移除未知狀態撥號名單項目時發生錯誤: ${removeError instanceof Error ? removeError.message : String(removeError)}`);
          }
      }
      ...
      if (previousCallRecord) {
        try {
          await CallListManager.removeUsedCallListItem(previousCallRecord.projectId, previousCallRecord.customerId);
          console.log(`[🟢 recordBonsaleCallResult] 🗑️ 已移除異常處理中的撥號名單項目 - 專案: ${previousCallRecord.projectId}, 客戶: ${previousCallRecord.customerId}`);
        } catch (removeError) {
          console.error(`[🟢 recordBonsaleCallResult] ❌ 移除異常處理中撥號名單項目時發生錯誤:`, removeError);
        }
      }
      const recordEndTime = Date.now();
      console.log(`[🟢 recordBonsaleCallResult 失敗] 耗時: ${recordEndTime - recordStartTime}ms`);
      // 不拋出錯誤，避免影響主要的外撥流程
    }

    const recordEndTime = Date.now();
    console.log(`[🟢 recordBonsaleCallResult 完成] 耗時: ${recordEndTime - recordStartTime}ms`);
  }
  ...
  private async checkAndReplenishCallList(): Promise<void> {
    try {
      ...
      console.log(`📊 專案 ${this.projectId} 撥號名單檢查 - 當前: ${currentCount}, 最低需求: ${minimumRequired} (分機數 ${this.agentQuantity} x 2)`);

      if (currentCount < minimumRequired) {
        console.log(`🔄 撥號名單不足，開始自動補充 - 專案: ${this.projectId}`);
        ...
        console.log(`✅ 撥號名單補充完成 - 專案: ${this.projectId}, 補充前: ${currentCount}, 補充後: ${newCount}`);
      } else {
        console.log(`✅ 撥號名單充足 - 專案: ${this.projectId}, 當前: ${currentCount}`);
      }
    } catch (error) {
      console.error(`❌ 檢查並補充撥號名單失敗 - 專案: ${this.projectId}:`, error);
      // 不拋出錯誤，避免影響主要的撥打流程
    }
  }
  ...
  private async getBonsaleOutboundCallList(): Promise<void> {
    try {
      console.log(`開始從 Bonsale API 獲取專案 ${this.projectId} 的撥號名單`);
      ...
      if (spaceLeft <= 0) {
        console.log(`🚫 撥號名單已達上限 - 專案: ${this.projectId}, 當前: ${currentCount}, 上限: ${maxAllowed}`);
        return;
      }
      ...
      console.log(`第一輪：獲取 callStatus = 0 的名單，限制 ${limit} 筆`);
      ...
      if (!firstOutboundResult.success) {
        console.error('第一輪獲取撥號名單失敗:', firstOutboundResult.error);
        return;
      }
      ...
      if (!firstList || firstList.length === 0) {
        ...
        console.log(`第一輪無結果，第二輪：獲取 callStatus = 2 的名單`);
        ...
        if (!secondOutboundResult.success) {
          console.error('第二輪獲取撥號名單失敗:', secondOutboundResult.error);
          return;
        }
        ...
        if (!secondList || secondList.length === 0) {
          console.warn('兩輪搜尋都無結果，所有名單已撥打完畢');
          return;
        }

        outboundList = secondList;
        console.log(`第二輪獲取到 ${secondList.length} 筆名單`);
      } else {
        outboundList = firstList;
        console.log(`第一輪獲取到 ${firstList.length} 筆名單`);
      }
      ...
      for (const item of outboundList) {
        ...
        if (exists) {
          console.log(`⚠️ 跳過重複客戶 - 客戶ID: ${item.customerId}, 姓名: ${item.customer?.memberName}`);
          continue;
        }
        ...
        if (validItems.length >= spaceLeft) {
          console.log(`✅ 已達到 Redis 存放上限 ${spaceLeft} 筆，停止過濾`);
          break;
        }
      }

      if (validItems.length === 0) {
        console.warn('過濾後沒有可用的新名單（全部重複或資料不完整）');
        return;
      }

      console.log(`📋 過濾結果 - 原始拉取: ${outboundList.length}/${limit}, 過濾後有效: ${validItems.length}, 將補充: ${Math.min(validItems.length, spaceLeft)}`);
      ...
      console.log(`✅ Bonsale 撥號名單補充完成 - 補充: ${successCount}/${itemsToAdd.length}, 失敗: ${failCount}, 最終總數: ${finalCount}/${maxAllowed}`);

      if (failCount > 0) {
        console.warn(`有 ${failCount} 筆資料添加失敗`);
        ...
        failedResults.forEach(({ result, index }) => {
          if (result.status === 'rejected') {
            console.error(`失敗項目 ${index + 1}:`, result.reason);
          }
        });
      }
      ...
      await this.setError(errorMsg);
      console.error('處理 Bonsale 撥號名單失敗:', error);
    }
  }
  ...
  private async handleWebSocketInitialization(broadcastWs?: WebSocketServer, context: string = '3CX WebSocket 連接成功'): Promise<void> {
    try {
      console.log(`🔗 3CX WebSocket ${context}`);

      // 檢查專案狀態，只有在 active 狀態下才執行初始化
      if (this.state !== 'active') {
        console.log(`📊 專案 ${this.projectId} 狀態為 ${this.state}，跳過 WebSocket 初始化`);
        return;
      }

      // 檢查並補充撥號名單
      console.log(`📋 檢查並補充撥號名單 - 專案: ${this.projectId}`);
      await this.checkAndReplenishCallList();

      // 執行外撥邏輯
      console.log(`📞 執行外撥邏輯 - 專案: ${this.projectId}`);
      // 使用 throttle 版本
      ...
      this.outboundCall(broadcastWs, null, true, true).catch(error => {
        console.error('異步執行初始外撥邏輯時發生錯誤:', error);
      });

      // 啟動空閒檢查定時器
      if (IS_STARTIDLECHECK) {
        console.log(`🕰️ 啟動空閒檢查定時器 - 專案: ${this.projectId}`);
        console.log(`🕰️ 空閒檢查參數 - 初始間隔: ${this.minIdleCheckInterval / 1000}秒, 最大間隔: ${this.maxIdleCheckInterval / 1000}秒`);
        console.log(`🕰️ 空閒檢查指數退避參數 - 乘數: ${this.idleCheckBackoffFactor}`);
        this.startIdleCheck(broadcastWs);
      } else {
        console.log(`⏸️ 未啟動空閒檢查定時器（IS_STARTIDLECHECK=${IS_STARTIDLECHECK}） - 專案: ${this.projectId}`);
      }

      console.log(`✅ WebSocket ${context} - 初始化完成`);
    } catch (error) {
      console.error(`❌ WebSocket ${context}後初始化時發生錯誤:`, error);
  ...
  private createWebSocketManagerConfig(broadcastWs?: WebSocketServer) {
    return {
      ...
      handlers: {
        onOpen: () => {
          console.log({ isForce: true }, '🔗 WebSocket 連接成功 - 完整專案資訊:', {
            projectId: this.projectId,
            callFlowId: this.callFlowId,
            state: this.state,
            client_id: this.client_id,
        ...
        onMessage: (data: Buffer) => {
          ...
          console.log('📨 3CX WebSocket data:', messageObject);
          console.log('📨 3CX WebSocket 收到訊息:', {
            projectId: this.projectId,
            callFlowId: this.callFlowId,
            state: this.state,
            client_id: this.client_id,
        ...
        onError: async (error: Error) => {
          ...
          await this.setError(errorMsg);
          console.error('3CX WebSocket 錯誤:', error);
        },
        onClose: (code: number, reason: Buffer) => {
          console.log(`3CX WebSocket 關閉: ${code} - ${reason.toString()}`);
        },
  ...
  private startIdleCheck(broadcastWs?: WebSocketServer): void {
    ...
    console.log(`🕰️ 專案 ${this.projectId} 空閒檢查定時器已啟動（指數退避機制，初始間隔：${this.idleCheckInterval / 1000}秒）`);
  }
  ...
  private stopIdleCheck(): void {
    if (this.idleCheckTimer) {
      ...
      console.log(`⏹️ 專案 ${this.projectId} 空閒檢查定時器已停止`);
    }
  }
    ...
    this.idleCheckTimer = setTimeout(async () => {
      try {
        ...
        if (hasIdleExtension) {
          ...
          this.idleCheckInterval = this.minIdleCheckInterval;
          console.log(`🔄 專案 ${this.projectId} 檢測到活動，重置檢查間隔為 ${this.idleCheckInterval / 1000} 秒`);
        } else {
          ...
          console.log(`⏰ 專案 ${this.projectId} 無活動，增加檢查間隔為 ${this.idleCheckInterval / 1000} 秒`);
        }
        ...
        console.error(`空閒檢查時發生錯誤 - 專案 ${this.projectId}:`, error);
        // 發生錯誤時也要安排下一次檢查
        if (this.state === 'active') {
          this.scheduleNextIdleCheck();
        }
    ...
    try {
      ...
      if (!this.hasActiveCalls()) {
        ...
        if (this.isCompleteStopScheduled) {
          console.log(`專案 ${this.projectId} 已排程完全停止，跳過重複排程`);
          return;
        }
        this.isCompleteStopScheduled = true;

        console.log(`專案 ${this.projectId} 已無活躍通話，執行完全停止`);
        ...
        console.log(`專案 ${this.projectId} 仍有活躍通話，等待通話結束`);
      }
    } catch (error) {
      console.error(`處理停止狀態邏輯時發生錯誤:`, error);
    }
  }
  ...
  private async processPendingCallRecords(): Promise<void> {
    try {
      console.log(`🔄 專案 ${this.projectId} 開始處理未完成的通話記錄`);

      // 檢查是否有未處理的 latestCallRecord
      if (this.latestCallRecord && this.latestCallRecord.length > 0) {
        console.log(`📞 發現 ${this.latestCallRecord.length} 筆未處理的通話記錄`);
        ...
        for (const callRecord of this.latestCallRecord) {
          if (callRecord) {
            ...
            console.log(`📋 移動通話記錄到待處理清單 - 分機: ${callRecord.dn}, 客戶: ${callRecord.memberName} (${callRecord.customerId})`);
          }
        }
      ...
      if (this.previousCallRecord && this.previousCallRecord.length > 0) {
        console.log(`🔄 開始處理 ${this.previousCallRecord.length} 筆待處理的通話記錄`);
        ...
        const processPromises = this.previousCallRecord
          ...
          .map(async (record) => {
            try {
              ...
              console.log(`✅ 完成處理通話記錄 - 分機: ${record!.dn}, 客戶: ${record!.memberName}`);
            } catch (error) {
              console.error(`❌ 處理通話記錄失敗 - 分機: ${record!.dn}, 客戶: ${record!.memberName}:`, error);
            }
          });
        ...
        console.log(`✅ 所有未完成的通話記錄處理完成`);
      } else {
        console.log(`ℹ️ 沒有待處理的通話記錄`);
      }

    } catch (error) {
      console.error(`處理未完成通話記錄時發生錯誤:`, error);
      // 不拋出錯誤，避免影響停止流程
    }
  }
  ...
  async executeCompleteStop(broadcastWs: WebSocketServer): Promise<void> {
    ...
    await this.processCallerMutex.runExclusive(async () => {
      ...
      if (this.isRemoved) {
        console.log(`專案 ${this.projectId} 已完全停止並移除，跳過重複執行`);
        return;
      }
      ...
      try {
        ...
        console.log(`🗑️ 清空專案 ${this.projectId} 的 Redis 暫存撥號名單`);
        const clearResult = await CallListManager.removeProjectCallList(this.projectId);
        if (clearResult) {
          console.log(`✅ 成功清空專案 ${this.projectId} 的撥號名單`);
        } else {
          console.warn(`⚠️ 清空專案 ${this.projectId} 撥號名單失敗`);
        }
        ...
        console.log(`專案 ${this.projectId} 已完全停止並移除`);

        // 通知外部（例如 activeProjects Map）移除此實例
        this.onCompleteStop?.();
      } catch (error) {
        console.error(`執行完全停止時發生錯誤:`, error);
      }
    });
  }
  ...
  private async handleTokenUpdateWebSocketReconnect(broadcastWs?: WebSocketServer): Promise<void> {
    if (this.wsManager && this.wsManager.isConnected() && this.access_token) {
      try {
        console.log('Token 已更新，重新建立 WebSocket 連接');
        await this.wsManager.disconnect();
        ...
        console.error('Token 更新後重連 WebSocket 失敗:', error);
      }
    }
  }
    ...
    try {
      ...
      if (runningProject) {
        ...
        if (!runningProject.hasActiveCalls()) {
          // 沒有活躍通話，立即執行完全停止
          console.log(`專案 ${projectId} 無活躍通話，立即完全停止`);
          await runningProject.executeCompleteStop(ws);
          activeProjects.delete(projectId);
        } else {
          // 有活躍通話，等待通話結束
          console.log(`專案 ${projectId} 有活躍通話，等待通話結束後自動停止`);
          // 廣播狀態更新
          await broadcastAllProjects(ws, projectId);
        }
      } else {
        // 如果沒有活躍實例，直接從 Redis 移除
        console.warn(`未找到活躍的專案實例: ${projectId}，直接從 Redis 移除`);
