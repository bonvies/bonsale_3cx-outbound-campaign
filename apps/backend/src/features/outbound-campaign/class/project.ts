...
export default class Project {
    ...
    await this.processCallerMutex.runExclusive(async () => {
      ...
      try {
        ...
        if (this.state !== 'active') {
          console.log('專案狀態不符合外撥條件:', this.state);
          return;
        }
  ...
  private async handleApiError(apiName: string, result: { success: boolean; error?: { error?: string } }, shouldThrow: boolean = true): Promise<boolean> {
    if (!result.success) {
      ...
      await this.setError(errorMsg);
      console.error({ isForce: true }, `❌ ${apiName} 錯誤:`, {
      ...
      console.error({ isForce: true }, errorMsg);
              ...
              for (let tryPost9000Times = 1; tryPost9000Times <= 3; tryPost9000Times++) {
                try {
                  ...
                  if (post9000Result.success) {
                    ...
                    if (isApiSuccess) {
                      console.log({ isForce: true }, `✅ post9000 成功 (第 ${tryPost9000Times} 次嘗試) - 電話: ${previousCallRecord.phone} - ${JSON.stringify(post9000Result.data)}`);
                      ...
                      if (tryPost9000Times < 3) {
                        ...
                        console.error({ isForce: true }, `❌ post9000 已達最大重試次數 (${tryPost9000Times} 次)，停止嘗試 - 電話: ${previousCallRecord.phone} - StatusCode=${apiData?.StatusCode}, Message=${apiData?.Message}`);
                      }
                    }
                  } else {
                    ...
                    if (tryPost9000Times < 3) {
                      ...
                      console.error({ isForce: true }, `❌ post9000 已達最大重試次數 (${tryPost9000Times} 次)，停止嘗試 - 電話: ${previousCallRecord.phone} - ${post9000Result.error?.error || '未知錯誤'}`);
                    }
                  }
                } catch (error) {
              ...
              if (post9000Success) {
                try {
                  ...
                  if (dummyResult.success) {
                    console.log({ isForce: true }, `✅ post9000Dummy 成功, ${JSON.stringify(dummyResult.data)}`);
                  } else {
          ...
          try {
            ...
            setTimeout(async () => {
              try {
                ...
                await this.setError(errorMsg);
                console.log({ isForce: true }, '❌ updateVisitRecord 異常:', {
                  projectId: this.projectId,
                  callFlowId: this.callFlowId,
                  state: this.state,
                  client_id: this.client_id,
