...
//
// WR 的 DT 是照 docs/FIAS_INTEGRATION.md 過去記錄的行為，但**不是**官方規格欄位
// 代碼——規格正式的叫醒記錄是 WR/WC/WA（日期欄位是 DA 不是 DT），可能是這台 PMS
// 的客製化欄位，也可能當初記錄的就是錯的、只是還沒被戳破，所以 DA/DT 都宣告、都
// 不漏接。RI/MR（重試間隔/次數）原本也宣告在這裡，但規格根本沒有這兩個欄位、且
// 實測證實 PMS 永遠不會填值，已移除宣告——重試改由 fiasHandler.ts 的
// FIAS_WR_RETRY_INTERVAL_MIN/FIAS_WR_MAX_RETRIES 環境變數決定，跟 PMS 送什麼無關。
// 取消叫醒原本用的 WD 確定不是官方代碼、也找不到任何文件根據，已直接移除，
// 只保留規格正式代碼 WC。
const LINK_RECORDS: { ri: string; fields: string }[] = [
  { ri: 'WR', fields: 'RNDADTTI' },      // 叫醒預約（From PMS，官方 DA + 目前假設的 DT 一併宣告）
