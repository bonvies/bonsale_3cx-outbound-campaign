...
//   STX (Start of Text) = 0x02，標記訊息開頭
//   ETX (End of Text)   = 0x03，標記訊息結尾
...
//
// FIAS 是 TCP 長連線，PMS 同時只會有一條連線。
// 將目前 active 的 conn 存在此處，讓 fiasHandler 以外的模組
//（例如 REST routes）也能主動送訊息給 PMS。
//
// 生命週期由 createServer 管理：
//   socket 建立後 → setActiveFiasConn
//   socket 關閉後 → clearActiveFiasConn
// ─────────────────────────────────────────────
let _activeConn: FiasConn | null = null;

export function setActiveFiasConn(conn: FiasConn): void {
  _activeConn = conn;
}

export function clearActiveFiasConn(): void {
  _activeConn = null;
}

/** 取得目前 active 的 FIAS 連線；若 PMS 尚未連線則回傳 null */
export function getActiveFiasConn(): FiasConn | null {
  return _activeConn;
}
...
export function createServer(handler: FiasHandler): net.Server {
  return net.createServer((socket) => {
    ...
    // 連線建立後登記 active conn，讓外部模組（e.g. REST routes）可以送訊息
    setActiveFiasConn(conn);
    ...
    socket.on('close', (hadError: boolean) => {
      ...
      // 連線關閉後清除 registry，避免外部誤用已失效的 conn
      clearActiveFiasConn();
    });
  });
}
